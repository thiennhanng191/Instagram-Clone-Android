...
object TimeFormatter {
    fun getTimeDifference(rawJsonDate: String?): String {
        ...
        val format =
            SimpleDateFormat(twitterFormat, Locale.ENGLISH)
        format.isLenient = true
        try {
            val diff =
                (System.currentTimeMillis() - format.parse(rawJsonDate).time) / 1000
            if (diff < 5) time = "Just now" else if (diff < 60) time = String.format(
                Locale.ENGLISH,
                "%ds",
                diff
            ) else if (diff < 60 * 60) time = String.format(
                Locale.ENGLISH,
                "%dm",
                diff / 60
     ...
     */
    fun getTimeStamp(rawJsonDate: Date): String {
        var time = ""
        val twitterFormat = "EEE MMM dd HH:mm:ss ZZZZZ yyyy"
        val format =
            SimpleDateFormat(twitterFormat, Locale.ENGLISH)
        ...
        try {
            ...
            val date = then.time
            val format1 =
                SimpleDateFormat("h:mm a \u00b7 M/d/yy")
